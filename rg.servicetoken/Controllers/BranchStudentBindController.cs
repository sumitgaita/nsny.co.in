...
{
    //[Authorize]
    [RoutePrefix("api/BranchStudentBind")]
    public class BranchStudentBindController : ApiController
    {
        private readonly IBranchStudentBindManager _branchStudentBind;
        private readonly IHttpResponseMessage _httpResponseMessage;

        public BranchStudentBindController(IHttpResponseMessage httpResponseMessage, IBranchStudentBindManager branchStudentBind)
        {
        ...
        public HttpResponseMessage Get(string stid)
        {
            try
            {
                System.Collections.Generic.List<BranchPaymentCollection> printDetails = _branchStudentBind.GetRecivedPrint(stid);
        ...
        [HttpPut]
        [Route("update")]
        public HttpResponseMessage StudentCourseBindUpdate([FromBody] JObject jsonData)
        {
            BranchStudentBind branchStudentBind = jsonData.ToObject<BranchStudentBind>();
            var response = _branchStudentBind.StudentCourseBindUpdate(branchStudentBind);
            return _httpResponseMessage.ReturnOk(response);
        }
